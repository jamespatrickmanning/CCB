...
@author: xiaojian
modifications by JiM in late August 2018
where there was some mistakes with bourne and plymouth being plotted the same as Sandwich
"""
import numpy as np
...
# NOTE: some towns start out with "2" so that they are at least visible
Hull=2
Nantasket_Beach=3
Hingham=3
Duxbury=3
...
for a in np.arange(len(ct1)):
    if ct1[a]=='Hull':
        ...
        plt.text(lonx[a]-0.025,latx[a]-0.01,'%s'%Truro,color='white')
    elif ct1[a]=="Wellfleet":
        plt.text(lonx[a]+0.05,latx[a]+0.0,ct1[a])
        plt.scatter(lonx[a],latx[a],s=Wellfleet,color='green')
        plt.text(lonx[a]-0.025,latx[a]-0.01,'%s'%Wellfleet,color='white')
    elif ct1[a]=="Falmouth":
        plt.text(lonx[a],latx[a]-0.04,ct1[a])
        #plt.scatter(lonx[a],latx[a],s=Sandwich,color='red',label='other towns(number=%s)'%(Falmouth))
        plt.scatter(lonx[a],latx[a],s=Falmouth,color='red',label='other towns')
   
    elif ct1[a]=="Eastham":
        plt.text(lonx[a]+0.05,latx[a]+0.0,ct1[a])
        #plt.scatter(lonx[a],latx[a],s=Eastham,color='green',label="'Outer Cape' towns(number=%s)"%(Eastham))
        plt.scatter(lonx[a],latx[a],s=Eastham,color='green',label="'Outer Cape' towns")
        plt.text(lonx[a]-0.025,latx[a]-0.01,'%s'%Eastham,color='white')
    elif ct1[a]=="Orleans":
        ...
        plt.scatter(lonx[a],latx[a],s=Dennis,color='#930093',label="'Mid Cape' towns")
    elif ct1[a]=="Sandwich":
        plt.text(lonx[a],latx[a]-0.04,ct1[a])
        ...
        plt.scatter(lonx[a],latx[a],s=Plymouth,color='red')
    elif ct1[a]=="Bourne":
        plt.text(lonx[a],latx[a]-0.04,ct1[a])
        #plt.scatter(lonx[a],latx[a],s=Sandwich,color='red')
        plt.scatter(lonx[a],latx[a],s=Bourne,color='red')
    else:
        continue
        """
        plt.text(lonx[a],latx[a]-0.04,ct1[a])
...
ax=plt.plot(CL['lon'],CL['lat'],'b-',linewidth=0.7)
...
#plt.ylabel('latitude')
plt.show()
plt.savefig('turtle_city(2012-2015).eps',format='eps',dpi=400,bbox_inches='tight')
#fig=plt.gcf()
#fig.savefig('fig4.eps',format='eps',dpi=400,bbox_inches='tight')
