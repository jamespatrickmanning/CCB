...
#    a=a[a<0]
#    if len(a)>1:
#        for b in np.arange(len(a)):
#            h.append(a[b])
#    else:
#        c=a[a>0]
#        for e in np.arange(len(c)):
#            h.append(c[e])
#    for v in np.arange(len(h)):
#        h[v]=h[v]/(np.array(h).sum())
#    h=w/w.sum()
...
# coordinates of the (dual mesh) polygon vertices: the centers of triangle faces
    ...
    #        print kv,kfv,Aj
            ...
            for kv1 in Grid['kvv'][0:Grid['nvv'][kv],kv]:
                ...
                if np.argwhere(Aj1<0).flatten().size==0:
                    ...
                    #print 'kv,kfv,Aj',kv,kfv,Aj
                    break
                
                '''
                else:
                    for kv2 in Grid['kvv'][0:Grid['nvv'][kv1],kv1]:
                        kfv1=Grid['kfv'][0:Grid['nfv'][kv2],kv2]
                        print 'kfv1',kfv1
                        lonv1=Grid['lonc'][kfv1];latv1=Grid['latc'][kfv1] 
                        w1,Aj1=polygonal_barycentric_coordinates(lonp,latp,lonv1,latv1)
                        ...
                        if np.argwhere(Aj1<0).flatten().size==0:
                            print 'in2'
                            w=w1;kfv=kfv1;kv=kv2;Aj=Aj1
                            print 'w,kv,kfv,Aj',w,kv,kfv,Aj
                            break
                '''
    ...
    #    for a in np.arange(len(w)):
    #        if w[a]<0:
                
        a=[]
        c=[]         #w[a]=0
        ww=np.array(w)
        #print 'size',np.argwhere(ww<0).flatten().size
        if np.argwhere(ww<0).flatten().size>0:
    #        print kv,kfv,w
            if np.argwhere(ww<0).flatten().size==len(ww):
                ww=np.abs(ww)
                for kk in np.arange(len(ww)):
                    c.append(ww[kk])
            else:
                #print 'in3'
        # set w=0 -> velocity=0 for points outside 
                for j in np.arange(len(ww)):
                    if ww[j]<0:
                        ww[j]=0
                for i in np.arange(len(ww)):
                    a.append(ww[i])
                b=0
                for d in np.arange(len(a)):
                    b=b+a[d]
                
                for d in np.arange(len(a)):
                    c.append((a[d]/float(b)))
        else:
            for k in np.arange(len(ww)):
                c.append(ww[k])
        ui=(u[kfv]*w).sum()
        vi=(v[kfv]*w).sum()
    else:
        ...
        dis=[]
        for aa in np.arange(len(lonv)):
            dis.append(1./np.sqrt((lonv[aa]-lonp)**2+(latv[aa]-latp)**2))
        sum1=np.sum(np.array(dis))
        uu=0
        vv=0
        for ab in np.arange(len(kfv)):
            uu=uu+u[kfv[ab]]*dis[ab]
            vv=vv+v[kfv[ab]]*dis[ab]
        ui=uu/float(sum1)
        vi=vv/float(sum1)
...
#        a=w[a]+a
#    if a==np.nan:
#        print 'jiaquanpingjunzhi'
#        for a in np.arange(len(lonv)):
#            dis[a]=1./np.sqrt((lonv[a]-lonp)**2+(latv[a]-latp)**2)
#        sum1=np.sum(dis)
#        uu=0
#        vv=0
#        for a in np.arange(len(kfv)):
#            uu=uu+u[kfv[a]]*dis[a]
#            vv=vv+v[kfv[a]]*dis[a]
#        ui=uu/float(sum1)
#        vi=vv/float(sum1)
...
class get_fvcom():
    ...
    def get_url(self, starttime, endtime):
        ...
        if self.modelname == "GOM3":
            ...
            Times = []
            for i in mTime:
                strt = '201'+i[3]+'-'+i[5]+i[6]+'-'+i[8]+i[9]+' '+i[11]+i[12]+':'+i[14]+i[15]#%Y-%m-%d %H:%M
            ...
            timeurl='''http://www.smast.umassd.edu:8080/thredds/dodsC/fvcom/archives/necofs_mb?Times[0:1:34655]'''
            url='''http://www.smast.umassd.edu:8080/thredds/dodsC/fvcom/archives/necofs_mb?
            lon[0:1:98431],lat[0:1:98431],lonc[0:1:165094],latc[0:1:165094],siglay[0:1:9][0:1:98431],h[0:1:98431],nbe[0:1:2][0:1:165094],
            zeta[{0}:1:{1}][0:1:98431],u[{0}:1:{1}][0:1:9][0:1:165094],v[{0}:1:{1}][0:1:9][0:1:165094]'''            
            
            """            
            timeurl = '''http://www.smast.umassd.edu:8080/thredds/dodsC/models/fvcom/NECOFS/Archive/NECOFS_MASS_2014/mbn_201411.nc?Times[0:1:719]'''
            url='''http://www.smast.umassd.edu:8080/thredds/dodsC/models/fvcom/NECOFS/Archive/NECOFS_MASS_2014/mbn_201411.nc?
            lon[0:1:98431],lat[0:1:98431],lonc[0:1:165094],latc[0:1:165094],siglay[0:1:9][0:1:98431],h[0:1:98431],zeta[{0}:1:{1}][0:1:98431],
            nbe[0:1:2][0:1:165094],u[{0}:1:{1}][0:1:9][0:1:165094],v[{0}:1:{1}][0:1:9][0:1:165094]'''
            """
            """
    ...
    def eline_path(self,lon,lat):
        ...
        for i in self.b_points:
            if p.contains_point(i):
                bps.append((i[0],i[1]))#boundary points
                d = math.sqrt((lon-i[0])**2+(lat-i[1])**2)#distance between point and boundary
                dis.append(d)
        bps = np.array(bps)
        if not dis:
            return None
            ...
            dnp = np.array(dis)
            dmin = np.argmin(dnp)#the index of minimum distance
            lonp = bps[dmin][0]; latp = bps[dmin][1]
            index1 = np.where(self.lonc==lonp)#the index of lon
            index2 = np.where(self.latc==latp)#the index of lat
            elementindex = np.intersect1d(index1,index2)[0]
    ...
    def nearpoint_index(self,lat,lon,year):
        dists=[];points=[]
        url_wind="""http://www.smast.umassd.edu:8080/thredds/dodsC/models/fvcom/NECOFS/Archive/meteorology_v1/m_forcing_%s.nc?XLAT[0:1:140][0:1:182],XLONG[0:1:140][0:1:182]"""  %(year)#%(year,ftime,stime,lats,lats,lons,lons)
    ...
    def get_track(self,lon,lat,depth,starttime,wind,wind_get_type): #,b_index,nvdepth, 
        ...
        windspeed=[]
        modpts = dict(lon=[lon], lat=[lat], layer=[], time=[], num_ashore=[]) #model forecast points
        ...
        #lonl,latl = self.shrink_data(lon,lat,self.lonc,self.latc,0.8)#1 day elements(blue)
        lonk,latk = self.shrink_data(lon,lat,self.lons,self.lats,1.2)#1 day nodes(red)   #have a circle and get the points in it
        try:
            ...
            index3 = np.where(self.lons==lonn)#index of lon of nearest point
            ...
            modpts['time'].append(self.mTime[0])#add the starttime
            ...
            if waterdepth<(abs(depth)): 
                ...
                num=1
                print 'num_ashore1:',num
                modpts['num_ashore'].append(num)
                raise Exception()
        except:
            return modpts
        ...
        for i in xrange(t):
            if i!=0 and i%24==0 :
                lonk,latk = self.shrink_data(lon,lat,self.lons,self.lats,1.2)
            u_t,v_t,kvdh=VelInterp_lonlat(lon,lat,Grid,self.u[i,layer],self.v[i,layer])
            #u_t----interpolated u 
            #v_t----interpolated v 
            #kvdh----the vertex corresponding to the dual mesh polygon
            ...
            if pa:
                ...
                if pa.intersects_path(tempa): 
                    ...
                    num=1#any number is ok ,just want to get the length of 'num_ashore'
                    print 'number_ashore2:',num
                    modpts['num_ashore'].append(num)
                    if len(windspeed)<2:
...
class get_roms():
    ...
    def shrink_data(self,lon,lat,lons,lats):
        '''have a circle and get the points in it'''
        lont = []; latt = []
